<template>
  <v-data-table
    :headers="headers"
    :items="animations"
    :server-items-length="animationNumber"
    ...
    <template v-slot:top>
      <v-toolbar flat>
        ...
        <v-dialog v-model="dialogForm" max-width="800px">
          ...
          <animation-minim-form :animation="editedItem" :operation="operation" @close:pop-up="closeForm" @reload="initialize" />
        </v-dialog>
...
export default {
  ...
  methods: {
    ...
    async sort () {
      const filterOptionTransformation = { status: 'status.name' }
      const tempFilterParams = await this.$helpers.filters.constructor(this.options, filterOptionTransformation)
      this.$api.animation.getFiltered(tempFilterParams)
    ...
    deleteItemConfirm () {
      ...
      this.animations.splice(this.editedIndex, 1)
    },
    ...
    variableAttribution (response) {
      ...
      this.animationNumber = response.data['hydra:totalItems']
      this.loading = false
